
const data = {
  title: "Oluwaseyi Oke Portfolio - Fullstack Software Engineer and Cloud Solutions Architect",
  description: "Oluwaseyi Oke is a Web Developer, Backend Engineer and Cloud Solutions Architect. This website is contains details about him and his work.",
  author: "Oluwaseyi Oke",
  keywords: "Web Development, Software Engineer, Backend Engineer, Cloud Solutions Architect, Fullstack Developer, Portfolio",
  domain: "https://oluwaseyi-oke-portfolio.vercel.app",
  image: "https://oluwaseyi-oke-portfolio.vercel.app/og-image.png",
  logo: "https://oluwaseyi-oke-portfolio.vercel.app/logo.png",
  phone: "+234-91-6039-7775",
  github: "https://github.com/iamArvy",
  portfolio: "https://oluwaseyi-oke-portfolio.vercel.app",
  linkedin: "https://linkedin.com/in/oluwaseyi-oke-backend-engineer"
}
export const meta = {
  title: data.title,
  description: data.description,
  image: data.image,
  keywords: data.keywords,
  author: data.author,
  viewport: "width=device-width, initial-scale=1",
  ogTitle: data.title,
  ogDescription: data.description,
  ogImage: data.image,
  ogUrl: data.domain,
  ogType: "websites",
  twitterCard: "summary_large_image",
  twitterTitle: data.title,
  twitterDescription: data.description,
  twitterImage: data.image
}
export const link = [{ rel: "canonical", href: data.domain }]
